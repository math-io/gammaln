...
function gammaln( x )  {
	...
	if ( x < 2 ) {
		...
		case 0:
			...
			p1 = A1C + z*(A1[0]+z*(A1[1]+z*(A1[2]+z*(A1[3]+z*A1[4]))));

			// TODO: replace with math-polynomial
			p2 = z * (A2C + z*(+A2[0]+z*(A2[1]+z*(A2[2]+z*(A2[3]+z*A2[4])))));
			p = y*p1 + p2;
			r += ( p - 0.5 * y );
			break;
		case 1:
			...
			// TODO: replace with math-polynomial
			p1 = T1C + w*(T1[0]+w*(T1[1]+w*(T1[2]+w*T1[3])));

			// TODO: replace with math-polynomial
			p2 = T2C + w*(T2[0]+w*(T2[1]+w*(T2[2]+w*T2[3])));
			p3 = T3C + w*(T3[0]+w*(T3[1]+w*(T3[2]+w*T3[3])));
			p = z*p1 - (TT - w*(p2+y*p3));
			r += ( TF + p );
			break;
		case 2:
			// TODO: replace with math-polynomial:
			p1 = y * (UC + y*(U[0]+y*(U[1]+y*(U[2]+y*(U[3]+y*U[4])))));

			// TODO: replace with math-polynomial
			p2 = VC + y*(V[0]+y*(V[1]+y*(V[2]+y*(V[3]+y*V[4]))));
			r += ( -0.5 * y + p1/p2 );
			break;
		}
	} 
	// 2 <= x < 8
		...
		// TODO: replace with math-polynomial
		p = y * (SC + y*(S[0]+y*(S[1]+y*(S[2]+y*(S[3]+y*(S[4]+y*S[5]))))));

		// TODO: replace with math-polynomial
		q = RC + y*(R[0]+y*(R[1]+y*(R[2]+y*(R[3]+y*(R[4]+y*R[5])))));
		r = 0.5*y + p/q;
	...
	// 8 <= x < 2**58
		...
		// TODO: replace with math-polynomial
		w = WC + z*(W[0]+y*(W[1]+y*(W[2]+y*(W[3]+y*(W[4]+y*W[5])))));
		r = (x-0.5)*(t-1) + w;
	}
	// 2**58 <= x <= Inf
	else {
